...
if [ ! -f "$INIT_FLAG" ]; then
    ...
    SQL="$SQL CREATE DATABASE IF NOT EXISTS \`$UPTIMEKUMA_DB\` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
    ...
    SQL="$SQL GRANT ALL PRIVILEGES ON \`$UPTIMEKUMA_DB\`.* TO '$MYSQL_USER'@'%';"
    SQL="$SQL FLUSH PRIVILEGES;"
    mysql -u root -e "$SQL"

    echo "[i] Importing kuma.sql into database '$UPTIMEKUMA_DB'..."
    mysql -u root "$UPTIMEKUMA_DB" < /usr/local/bin/kuma.sql
